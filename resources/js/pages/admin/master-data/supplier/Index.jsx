...
export default function SupplierIndexPage() {
    ...
    const fetchRows = async (page = 1) => {
        ...
        try {
            const response = await axios.get("/admin/supplier", {
                params: {
                    page,
    ...
    const handleSubmit = async (e) => {
        ...
        const payload = {
            ...
            contact_supplier: form.contact_supplier || null,
            alamat: form.alamat || null,
        };

        try {
            if (isEdit) {
                await axios.put(`/admin/supplier/${editingId}`, payload);
                setSuccess("Supplier berhasil diperbarui.");
            } else {
                await axios.post("/admin/supplier", payload);
    ...
    const handleDelete = async (row) => {
        const confirmed = window.confirm(
            `Hapus supplier "${row.nama_supplier}"?`
        );
        ...
        try {
            await axios.delete(`/admin/supplier/${row.id}`);
                    ...
                    <div className="flex items-start gap-4">
                        ...
                        <div>
                            ...
                            <p className="mt-2 max-w-3xl text-sm font-semibold leading-6 text-slate-500">
                                Kelola data supplier untuk kebutuhan pembelian,
                                inbound barang, dan stok event POS.
                            </p>
                        </div>
                    </div>
                    ...
                    <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
                        <div>
                            ...
                            <p className="mt-1 text-sm font-semibold text-slate-500">
                                Menampilkan data supplier yang tersimpan.
                            </p>
                        </div>
                                            ...
                                            return (
                                                ...
                                                >
                                                    ...
                                                    <td className="whitespace-nowrap px-5 py-4 font-semibold text-slate-500">
                                                        {row.contact_supplier || "-"}
                                                    </td>

                                                    <td className="min-w-[220px] px-5 py-4 font-semibold text-slate-500">
                                                        {row.alamat || "-"}
                                                    </td>
                            ...
                            <form onSubmit={handleSubmit} className="space-y-5">
                                ...
                                <div>
                                    ...
                                    <textarea
                                        ...
                                        rows="4"
                                        className="w-full resize-none rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3 text-sm font-semibold text-slate-700 outline-none transition placeholder:text-slate-400 focus:border-slate-950 focus:bg-white"
                                        disabled={saving}
                                    />
                                </div>
...
function LoadingRows() {
    return Array.from({ length: 5 }).map((_, index) => (
        <tr key={index} className="animate-pulse border-t border-slate-100">
            ...
            <td className="px-5 py-4">
                <div className="h-4 w-44 rounded bg-slate-100"></div>
            </td>
