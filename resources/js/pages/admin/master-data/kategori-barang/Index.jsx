...
export default function KategoriBarangIndexPage() {
    ...
    const fetchRows = async (page = 1) => {
        ...
        try {
            const response = await axios.get("/admin/kategori-produk", {
                params: {
                    page,
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            if (isEdit) {
                await axios.put(`/admin/kategori-produk/${editingId}`, form);
                setSuccess("Kategori produk berhasil diperbarui.");
            } else {
                await axios.post("/admin/kategori-produk", form);
                setSuccess("Kategori produk berhasil ditambahkan.");
            }
    ...
    const handleDelete = async (row) => {
        const confirmed = window.confirm(
            `Hapus kategori produk "${row.kategori}"?`
        );
        ...
        try {
            await axios.delete(`/admin/kategori-produk/${row.id}`);
            setSuccess("Kategori produk berhasil dihapus.");
            fetchRows(pagination.current_page);
        } catch (err) {
            console.error(err);
            setError("Gagal menghapus kategori produk.");
        } finally {
            setDeletingId(null);
        }
    };
                    ...
                    <div className="flex items-start gap-4">
                        ...
                        <div>
                            ...
                            <p className="mt-2 max-w-3xl text-sm font-semibold leading-6 text-slate-500">
                                Kelola kategori produk sebagai dasar pengelompokan
                                barang, harga, stok, dan transaksi event pada sistem
                                POS.
                            </p>
                        </div>
                    </div>
                    ...
                    <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
                        <div>
                            ...
                            <p className="mt-1 text-sm font-semibold text-slate-500">
                                Menampilkan daftar kategori produk yang tersimpan.
                            </p>
                        </div>
                            ...
                            <form onSubmit={handleSubmit} className="space-y-5">
                                <div>
                                    ...
                                    <p className="mt-2 text-xs font-semibold text-slate-400">
                                        Nama kategori digunakan untuk mengelompokkan
                                        produk pada master data.
                                    </p>
                                </div>
